package com.rc.droid_stalker.thread_factory;
...
public class EfficientThreadPoolExecutor extends ThreadPoolExecutor {
     ...
     * parameters and default thread thread_factory and rejected execution handler.
     * It may be more convenient to use one of the {@link java.util.concurrent.Executors} thread_factory
    ...
    /**
     * Creates a new {@code ThreadPoolExecutor} with the given initial
     * parameters and default thread thread_factory.
     *
     * @param corePoolSize    the number of threads to keep in the pool, even
     *                        if they are idle, unless {@code allowCoreThreadTimeOut} is set
     * @param maximumPoolSize the maximum number of threads to allow in the
     ...
     *                        executed.  This queue will hold only the {@code Runnable}
     *                        tasks submitted by the {@code execute} method.
     * @param threadFactory   the thread_factory to use when the executor
     ...
     *       try {
     *         Object result = ((Future<?>) r).initialize();
